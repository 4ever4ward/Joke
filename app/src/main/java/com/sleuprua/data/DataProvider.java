...
public class DataProvider {
    ...
    public void deleteFromFavourites(Joke joke) {
        HttpClient httpclient = new DefaultHttpClient();
        HttpPost httppost = new HttpPost(API_URL + "delete-favorite");

        String dfavouritesJsonStr = "";
        ...
        try {
            ...
            dfavouritesJsonStr = inputStreamToString(response.getEntity().getContent());
            Log.v(TAG, "deleteFromFavourites: " + dfavouritesJsonStr);


        } catch (ClientProtocolException e) {
            Log.e(TAG, "deleteFromFavourites: Problem with Internet connection", e);
        } catch (UnsupportedEncodingException e) {
            Log.e(TAG, "deleteFromFavourites: UnsupportedEncodingException", e);
        } catch (IOException e) {
            Log.e(TAG, "deleteFromFavourites: IOException", e);
        }


        deleteFavourite(joke);


    }

    public boolean setJokeVote(int jokeID, int jokeVoteID) {
        ...
        try {
            ...
            nameValuePairs.add(new BasicNameValuePair(PARAM_UUID, getUUID()));
            nameValuePairs.add(new BasicNameValuePair(PARAM_JOKE_ID, Integer.toString(jokeID)));

            httppost.setEntity(new UrlEncodedFormEntity(nameValuePairs));
            HttpResponse response = httpclient.execute(httppost);
            String voteJsonStr = inputStreamToString(response.getEntity().getContent());
            Log.v(TAG, "setJokeVote: " + voteJsonStr);


            updateJoke(jokeID,
                    AppDBContract.JokeEntry.TABLE_NAME,
                    AppDBContract.JokeEntry.COLUMN_JOKE_VOTE,
                    Integer.toString(jokeVoteID));


            if (voteJsonStr.length() != 0) {
                return true;
            }
        ...
        updateJoke(jokeID,
                AppDBContract.JokeEntry.TABLE_NAME,
                AppDBContract.JokeEntry.COLUMN_JOKE_TEXT,
                "valvlalvallva");

        return false;
    }

    public boolean addToFavourites(Joke joke) {

        HttpClient httpclient = new DefaultHttpClient();
        HttpPost httppost = new HttpPost(API_URL + "favorite");

        String favouritesJsonStr = "";
        ...
        try {
            ...
            HttpResponse response = httpclient.execute(httppost);
            favouritesJsonStr = inputStreamToString(response.getEntity().getContent());
            Log.v(TAG, "addToFavourites: " + favouritesJsonStr);
        ...
        updateJoke(joke.getJokeID(),
                AppDBContract.JokeEntry.TABLE_NAME,
                AppDBContract.JokeEntry.COLUMN_JOKE_FAVOURITE,
                Integer.toString(Joke.FAVOURITES_TRUE));
...
//                Integer.toString());
        ...
        return favouritesJsonStr.length() != 0;
    ...
    public ArrayList<Joke> getJokesByCategory(int categoryID) {
        ...
        return Utility.deleteUsedJokes(getAllJokesByCategoryFromDB(categoryID));
    }
    ...
    public ArrayList<Joke> getFavourites() {
        ...
        try {
            ...
            HttpResponse response = httpclient.execute(httppost);
            String jokeJsonStr = inputStreamToString(response.getEntity().getContent());
            getFavouriteDataFromJson(jokeJsonStr);
            ...
            try {
                ...
                if (getJokeByID(jokeID, AppDBContract.JokeEntry.TABLE_NAME) == null) {
                    ...
                    updateJoke(new Joke(jokeID,
                            ...
                            jokeLikes,
                                    jokeDislikes,
                                    getJokeByID(jokeID, AppDBContract.JokeEntry.TABLE_NAME).getJokeFavorites(),
                                    getJokeByID(jokeID, AppDBContract.JokeEntry.TABLE_NAME).getMyJokeVote()),
                            AppDBContract.JokeEntry.TABLE_NAME);
                }
    ...
    private void getFavouriteDataFromJson(String favouritesJsonStr) throws JSONException {
        final String ID = "id";
        final String DATE = "date_added";
        final String TEXT = "text";
        final String LIKES = "count_likes";
        ...
        JSONObject dataJsonObj = new JSONObject(favouritesJsonStr);
        JSONObject responseArrayJson = dataJsonObj.getJSONObject(RESPONSE);

        Iterator x = responseArrayJson.keys();
        JSONArray jokeListJsonArr = new JSONArray();

        // Part of code where we transform JSONObject to JSONArray
        while (x.hasNext()) {
            String key = (String) x.next();
            jokeListJsonArr.put(responseArrayJson.get(key));
        }
        ...
        for (int i = 0; i < jokeListJsonArr.length(); i++) {
            ...
            try {
                ...
                // If not exists joke with jokeID then add new joke
                if (getJokeByID(jokeID, AppDBContract.FavouritesEntry.TABLE_NAME) == null) {

                    // jokeCategory may have inside more than 1 categories
                    addFavourites(new Joke(jokeID,
                            jokeDate.getTimeInMillis(),
                            jokeText,
                            jokeLikes,
                            jokeDislikes, 0, 0));
                    ...
                    updateJoke(new Joke(jokeID,
                            ...
                            jokeText,
                            jokeLikes,
                            jokeDislikes, 0, 0), AppDBContract.FavouritesEntry.TABLE_NAME);
                }

            } catch (ParseException e) {
                Log.e(TAG, "getFavouriteDataFromJson: ParseException", e);
            }
    ...
    private ArrayList<Joke> getAllJokesByCategoryFromDB(int categoryID) {
        ...
        jokesList.add(new Joke(-1, 0, null, 0, 0, 0, 0));
    ...
    public ArrayList<DeferredOperation> getAllDeferredOperationFromDB() {
        SQLiteDatabase db = appDBHelper.getReadableDatabase();

        ArrayList<DeferredOperation> deferredOperations = new ArrayList<>();

        String query = "SELECT * FROM "
                + AppDBContract.DeferredOperationEntry.TABLE_NAME;
        ...
        if (cursor.moveToFirst()) {
            do {
                ...
                deferredOperations.add(deferredOperation);
            } while (cursor.moveToNext());
        }
    ...
    private void updateJoke(Joke joke, String tableName) {
    ...
    public void updateJoke(int jokeId, String tableName, String col, String val) {
        SQLiteDatabase db = appDBHelper.getWritableDatabase();
        ...
        db.update(tableName, contentValues,
                AppDBContract.JokeEntry.COLUMN_ID + " = ?", new String[]{String.valueOf(jokeId)});
    ...
    private void addJoke(Joke joke, String categoryIDs) {
        ...
        contentValues.put(AppDBContract.JokeEntry.COLUMN_JOKE_LIKES, joke.getJokeLikes());
        contentValues.put(AppDBContract.JokeEntry.COLUMN_JOKE_DISLIKES, joke.getJokeDislikes());
        contentValues.put(AppDBContract.JokeEntry.COLUMN_JOKE_FAVOURITE, 0);
        contentValues.put(AppDBContract.JokeEntry.COLUMN_JOKE_VOTE, 0);
    ...
    public void addDeferredOperation(DeferredOperation deferredOperation) {
        SQLiteDatabase db = appDBHelper.getWritableDatabase();
        ContentValues contentValues = new ContentValues();

        contentValues.put(AppDBContract.DeferredOperationEntry.COLUMN_JOKE_ID, deferredOperation.getJokeID());
        contentValues.put(AppDBContract.DeferredOperationEntry.COLUMN_OPERATION_ID, deferredOperation.getOperationID());

        db.insert(AppDBContract.DeferredOperationEntry.TABLE_NAME, null, contentValues);
        db.close();

    }

    public void deleteDeferredOperation(DeferredOperation deferredOperation) {
        SQLiteDatabase db = appDBHelper.getWritableDatabase();

        db.delete(AppDBContract.DeferredOperationEntry.TABLE_NAME,
                AppDBContract.DeferredOperationEntry.COLUMN_JOKE_ID + " = ?" +
                        " AND " +
                        AppDBContract.DeferredOperationEntry.COLUMN_OPERATION_ID + " = ?",
                new String[]{Integer.toString(deferredOperation.getJokeID()),
                        Integer.toString(deferredOperation.getOperationID())});
    }

    public void deleteFavourite(Joke joke) {
        SQLiteDatabase db = appDBHelper.getWritableDatabase();

        db.delete(AppDBContract.FavouritesEntry.TABLE_NAME,
                AppDBContract.FavouritesEntry.COLUMN_ID + " = ?",
                new String[]{Integer.toString(joke.getJokeID())});
    }
    ...
    private String getUUID() {
        ...
        return packageName + "::" + "bbaabbaabbaaabbb";//device_uuid;
    }
